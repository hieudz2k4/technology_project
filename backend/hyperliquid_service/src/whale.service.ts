...
export class WhaleService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    setInterval(() => void this.updateKnownTraders(), 1000 * 60 * 10); // Refresh every 10 mins
    setInterval(() => void this.fetchUniverse(), 1000 * 60 * 60); // Refresh assets every 1 hour
  ...
  private async fetchUniverse() {
    try {
      ...
      const response = await lastValueFrom(
        this.httpService.post<UniverseResponse>(
      ...
      if (response.data && response.data.universe) {
        const newAssets: string[] = response.data.universe.map(
          (asset) => asset.name,
        );
        ...
        this.logger.log(
          `Universe updated. Total assets: ${this.subscribedAssets.size}. New: ${newlyAddedCount}`,
        );
      }
    } catch (error) {
  ...
  private connect() {
    if (
      ...
      this.logger.warn(
        'WebSocket is already open or connecting. Skipping connect() call.',
      );
      return;
    }

    this.logger.log(
      `Connecting to Hyperliquid WebSocket... (Attempt ${this.reconnectAttempts + 1})`,
    );
    ...
    this.ws.on('close', (code, reason) => {
      this.logger.warn(
        `Disconnected from Hyperliquid WS. Code: ${code}, Reason: ${reason.toString()}`,
      );
      this.ws = null;
      this.stopPing();
    ...
    this.pingInterval = setInterval(() => {
      if (this.ws?.readyState === WebSocket.OPEN) {
        ...
        if (timeSinceLastPong > 45000) {
          // 45s threshold (1.5x ping interval)
          this.logger.warn(
            `No heartbeat received for ${timeSinceLastPong}ms. Terminating connection.`,
          );
      ...
      if (message.channel === 'trades' && message.data) {
        ...
        groups.forEach((trades) => {
          ...
          if (totalValueUsd >= this.WHALE_THRESHOLD_USD || isKnownTrader) {
            ...
            if (isKnownTrader) {
              this.logger.log(
                `Targeted Trader Detected! ${coin} ${side} $${totalValueUsd.toFixed(2)}`,
              );
            } else {
              this.logger.log(
                `Aggregated Whale Trade: ${coin} ${side} $${totalValueUsd.toFixed(2)} from ${trades.length} fills`,
              );
            }

            const aggregatedTrade: WhaleTradeData = {
              coin,
              side,
            ...
            void this.saveTradeToDb(aggregatedTrade);
        ...
        if (message.channel === 'subscriptionResponse') {
          // reduce log noise
          this.logger.debug('Subscription response received');
        } else if (message.channel === 'error') {
          this.logger.error(
            `Received error message from WS: ${JSON.stringify(message)}`,
          );
  ...
  private async saveTradeToDb(tradeData: WhaleTradeData) {
    try {
      ...
      this.logger.error('Failed to save whale trade to DB', error);
    }
  }
  ...
  private async updateKnownTraders() {
    try {
      ...
      this.logger.log(
        `Updated known traders cache. Total: ${this.knownTraders.size}`,
      );
    } catch (error) {
