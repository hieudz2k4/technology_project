...
export class TrongridService implements OnModuleInit {
    ...
    onModuleInit() {
        this.logger.log('Trongrid Service initialized. Polling for large USDT transfers...');
    ...
    async pollTransactions() {
        ...
        try {
            ...
            transactions.sort((a: any, b: any) => a.block_timestamp - b.block_timestamp);
            ...
            const errorMsg = `Error polling TronGrid: ${error.message}`;
            this.logger.error(errorMsg);
            try {
                await this.pushoverService.sendNotification(`⚠️ <b>TronGrid Polling Error</b> ⚠️\n${error.message}`, 'TronGrid Error', 'intermission');
            } catch (notifyError) {
    ...
    private async processTransaction(tx: any) {
        ...
        try {
            // Check if exists
            const exists = await this.transferRepository.findOne({ where: { hash: hash } });
            if (!exists) {
        ...
        const formattedAmount = amountDisplay.toLocaleString('en-US', { maximumFractionDigits: 0 });
        const typeDisplay = type === 'OUTFLOW' ? '<b>OUTFLOW</b> (Burn/Transfer)' : (type === 'INFLOW' ? '<b>INFLOW</b> (Mint/Receive)' : 'UNKNOWN');

        const msg = `🚨 <b>Tether Treasury Alert</b> 🚨\n` +
            `Type: ${typeDisplay}\n` +
        ...
        this.logger.log(`Large Transfer detected: ${formattedAmount} USDT - ${hash}`);
        ...
        try {
            await this.pushoverService.sendNotification(msg, 'Tether Treasury Alert', 'cashregister');
        } catch (err) {
