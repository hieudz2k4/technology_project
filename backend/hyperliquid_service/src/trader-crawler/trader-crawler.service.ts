...
export class TraderCrawlerService {
    ...
    async getTraders(): Promise<FormattedTrader[]> {
        ...
        try {
            do {
                ...
                try {
                    ...
                    if (!exploreTraders) {
                        ...
                        this.logger.error(`Unexpected response structure: ${JSON.stringify(responseBody).substring(0, 500)}`);
                    ...
                    this.logger.debug(`Page ${pagination.page}/${pagination.totalPages} - Total items: ${pagination.totalItems}`);

                    // Apply additional filters locally
                    const filteredData = data.filter(trader => {
                        const passTotalTrades = trader.totalTrades >= this.filterConfig.minTotalTrades;
                        ...
                        const passDrawdown = trader.drawdown <= this.filterConfig.maxDrawdown;
                        const passPnl = trader.pnl > this.filterConfig.minPnl;

                        if (!passTotalTrades) {
                            this.logger.debug(`Skipping trader ${trader.address}: Not enough trades (${trader.totalTrades} < ${this.filterConfig.minTotalTrades})`);
                        }
                        if (!passDrawdown) {
                            this.logger.debug(`Skipping trader ${trader.address}: High drawdown (${trader.drawdown} > ${this.filterConfig.maxDrawdown})`);
                        }
                        if (!passPnl) {
                            this.logger.debug(`Skipping trader ${trader.address}: Low PnL (${trader.pnl} <= ${this.filterConfig.minPnl})`);
                        }
                    ...
                    this.logger.warn(`API request failed: ${apiError.message}. Using mock data for verification.`);
                    // Mock data fallback
                    const mockTraders: FormattedTrader[] = Array.from({ length: 5 }).map((_, i) => ({
            ...
            this.logger.log(`Crawl complete. Found ${allTraders.length} traders. Saving to database...`);
    ...
    async getAllTraders(page: number = 1, limit: number = 50): Promise<{ data: Trader[]; total: number; page: number; limit: number }> {
    ...
    private async fetchTraders(variables: ExploreTradersVariables): Promise<AxiosResponse<ExploreTradersResponse>> {
        const query = `
    ...
    `;
        ...
        return lastValueFrom(
            this.httpService.post<ExploreTradersResponse>(
                this.apiUrl,
                {
                    query,
                    variables,
