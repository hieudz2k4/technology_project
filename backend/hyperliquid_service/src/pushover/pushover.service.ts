...
export class PushoverService {
    ...
    async sendNotification(message: string, title?: string, sound?: string, priority?: number, retry?: number, expire?: number): Promise<any> {
        ...
        try {
            ...
            this.logger.log(`Notification sent successfully: ${JSON.stringify(response.data)}`);
            return response.data;
        } catch (error) {
            this.logger.error(`Failed to send Pushover notification: ${error.message}`);
            if (axios.isAxiosError(error) && error.response) {
                this.logger.error(`Pushover API response: ${JSON.stringify(error.response.data)}`);
                throw new Error(`Pushover API Error: ${JSON.stringify(error.response.data)}`);
            }
            throw error;
        }
    }
